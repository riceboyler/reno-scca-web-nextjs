...
export type AbsoluteCenterVariantProps = {
  [key in keyof AbsoluteCenterVariant]?:
    | ConditionalValue<AbsoluteCenterVariant[key]>
    | undefined;
};
...
export interface AbsoluteCenterRecipe {
  ...
  splitVariantProps<Props extends AbsoluteCenterVariantProps>(
    props: Props
  ): [
    AbsoluteCenterVariantProps,
    Pretty<DistributiveOmit<Props, keyof AbsoluteCenterVariantProps>>,
  ];
  getVariantProps: (
    props?: AbsoluteCenterVariantProps
  ) => AbsoluteCenterVariantProps;
}
